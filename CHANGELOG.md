...
- **主题首帧防闪烁**：`index.html` 内联脚本在样式加载前读取 `s-enc-theme` 并设置 `data-theme`，主题变量内联预置——浅色/深色主题下刷新不再出现黑白闪变
...
- **文本加密解密恢复名**：解密下载名 `text.enc` → `text.txt`（容器头 originalFilename 现记录明文扩展名，加密产物文件名仍为 `text.enc`）
- **解密文本预览中文乱码**：400 字节截窗可能切断 UTF-8 多字节字符，fatal 解码失败回退 GB18030 产生乱码；现截取前先按 UTF-8 字符边界对齐，并按 code point 取前 100 字符
