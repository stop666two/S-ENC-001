...
| 双语界面 | 中文 / English 即时切换并持久化，语言自动检测 |
| 主题 | 自动（跟随系统）/ 浅色 / 深色 三态切换 |
...
> 加密产物扩展名为 `.enc`（文本加密为 `text.enc`）。密文直接打开必然乱码——请使用本工具解密。解密下载时按容器内记录的原文件名恢复：文件加密恢复原文件名（含扩展名），文本加密恢复为 `text.txt`。
